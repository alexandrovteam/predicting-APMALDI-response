...
def train_test_regression_models_multi_output(train_x, test_x, train_y, test_y,
    ...
    for r in pbar:
        ...
        if "MultiOut" not in r:
            regressor = MultiOutputRegressor(regressor)
        if train:
            regressor.fit(train_x, train_y)
            y_pred = regressor.predict(test_x)

            loc_res_df = pd.DataFrame(test_y, columns=out_multi_index).stack([i for i in range(len(out_multi_index.levels))]).reset_index().drop(columns=["level_0"]).rename(columns={0: "observed_value"})
            loc_res_df['prediction'] = pd.DataFrame(y_pred, columns=out_multi_index).stack([i for i in range(len(out_multi_index.levels))]).reset_index()[0]
...
def train_with_cross_val_multi_output(features_normalized, intensity_column="spot_intensity"):
    ...
    for train_index, test_index in pbar_cross_split:
        ...
        train_x = np.array([rows.iloc[0].to_numpy() for _, rows in g_train[adducts_columns.tolist() + features_normalized.columns.tolist()]])

        test_y = np.array([rows.to_numpy() for _, rows in g_test[intensity_column]])
        test_x = np.array([rows.iloc[0].to_numpy() for _, rows in g_test[adducts_columns.tolist() + features_normalized.columns.tolist()]])
...
def train_classification_models_multi_output(train_x, test_x, train_y, test_y,
    ...
    if name_test is not None and is_multioutput:
        nb_outputs = test_y.shape[1]
        name_test = name_test.loc[name_test.index.repeat(nb_outputs)]
    ...
    for r in pbar:
        ...
        regressor = regressors[r]
        # These classifiers need to train several models (one per class):
        if "MultiOut" not in r and is_multioutput:
            regressor = MultiOutputClassifier(regressor)
        elif "MultiOut" in r and (not test_multioutout_models or not is_multioutput):
...
if task_type == "regression":
    ...
    tick = time.time()
    print("Both features")
    regression_results_all_feat = train_detection_classifiers(features_norm_df)
